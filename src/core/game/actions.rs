...
impl Game {
    ...
    pub fn execute_action(&mut self, rule_engine: &crate::rules::RuleEngine, action: &crate::rules::GameAction) -> Result<(), Vec<crate::rules::RuleViolation>> {
        ...
        match action {
            ...
            crate::rules::GameAction::PlayCard { player_id, card_id, target: _ } => {
            ...
            crate::rules::GameAction::AttachEnergy { player_id, energy_id, pokemon_id } => {
                if let Some(player) = self.players.get_mut(player_id) {
                    if player.attach_energy(*energy_id, *pokemon_id) {
            ...
            crate::rules::GameAction::UseAttack { player_id, pokemon_id, attack_index } => {
            ...
            crate::rules::GameAction::Retreat { player_id: _, pokemon_id: _ } => {
...
mod tests {
    ...
    fn test_execute_action_draw_card() {
        use crate::rules::{RuleEngine, GameAction};
    ...
    fn test_execute_action_end_turn() {
        ...
        let action = GameAction::EndTurn { player_id: player1_id };
